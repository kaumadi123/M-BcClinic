...
if(isset($_POST["user_to"]) && !empty($_POST["user_to"]))
{
   ...
   if(mysql_num_rows($result) > 0)
   {
	   ...
	   ?>
       <a href="javascript:void(0);" onclick="startNewChatEstablishment('<?php echo strip_tags($get_users_fullnames["username"]); ?>','<?php echo nformatnamdesn(strip_tags($get_users_fullnames["fullname"])); ?>');" id="" class="vasplus_tooltips"><img src="vasplus_programming_blog_chat_application/smileys/notice.gif" style="margin-top:5px;" border="0" align="absmiddle" /><span style="font-family:Verdana, Geneva, sans-serif; font-size:11px; color:black;">New chat from <b><?php echo nformatnamdesn(strip_tags($get_users_fullnames["fullname"])); ?></b></span></a>
       <?php
   }
   else
   {
