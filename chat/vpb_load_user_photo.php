...
if(isset($_POST["username"]) && !empty($_POST["username"])) //Be sure that there is a valid session created for user photo check
{
	...
	echo "<span class='nprofileimg'><a href='javascript:void(0);'><img src='vasplus_programming_blog_chat_application/photos/".$userPic."' alt='".strip_tags($get_userPic["fullname"])."' title='".strip_tags($get_userPic["fullname"])."' border='0' style='width:100px;height:90px;'></a></span>"; 
}
else
{
	//No valid session username brought therefore, show the default users avatar
	echo "<span class='nprofileimg'><a href='javascript:void(0);'><img src='vasplus_programming_blog_chat_application/photos/avatar.gif' alt='' title='' border='0' style='width:100px;height:90px;'></a></span>";
}
?>