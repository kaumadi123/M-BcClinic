...
if(isset($_POST["page"]) && !empty($_POST["page"])) //Page Validation Process
{
	...
	if($_POST["page"] == "new_todo_item") 
	{
		...
		{
			if(mysql_query("insert into `to_do_list_mother` values('', '".mysql_real_escape_string($username)."', '".mysql_real_escape_string(remove_magic_quotes($new_todo_item))."', '".mysql_real_escape_string(date('d-m-Y'))."')"))
			{
				$check_recently_added_item = mysql_query("select * from `to_do_list_mother` where `username` = '".mysql_real_escape_string($username)."' order by `id` desc limit 1");
	...
	{
		mysql_query("delete from `to_do_list_mother` where `id` = '".mysql_real_escape_string(strip_tags($_POST["id"]))."'");
		$check_all_added_item = mysql_query("select * from `to_do_list_mother`");
	...
	elseif($_POST["page"] == "save_todo_item") 
	{
		...
		else
		{
			...
			if(mysql_query("update `to_do_list_mother` set `content` = '".mysql_real_escape_string(remove_magic_quotes($vpb_todo_item_brought))."' where `id` = '".mysql_real_escape_string(strip_tags($_POST["id"]))."'"))
			{
				$check_recently_added_item = mysql_query("select * from `to_do_list_mother` where `id` = '".mysql_real_escape_string(strip_tags($_POST["id"]))."'");
				if(mysql_num_rows($check_recently_added_item) < 1)
				{
